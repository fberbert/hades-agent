...
- busca web, captura de tela, anexos de imagem, chamadas de ferramentas de IA, skills locais, tarefas agendadas e consolidação de memória em segundo plano.
...
MiniChat é a janela de conversa do assistente (`?window=chat`). Ele gerencia estado do chat, chama Gemini, executa ferramentas, contabiliza tokens e pode ser fixado.
...
Susurro é o modo de transcrição e tradução ao vivo (`?window=susurro`). Ele captura áudio do sistema quando a plataforma suporta, envia chunks PCM ao Gemini Live, recebe deltas de transcrição, opcionalmente traduz texto e pode gerar sugestões a partir do transcript.
...
O fluxo de voz (`?window=voice`) grava áudio, converte para WAV/base64, envia por IPC e insere a transcrição no chat.
...
Settings gerencia chaves de API, modelos, áudio, stealth mode, histórico e atalhos globais.
...
- SDKs de IA: `@google/genai`, `@google/generative-ai`.
- Busca: Tavily via `axios`.
